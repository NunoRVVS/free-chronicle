...
export default function HomePage(props) {
  ...
  return (
    <main className='flex-1 p-4 flex flex-col text-center gap-3 sm:gap-4 justify-center pb-20'>
        ...
        md:text-7xl'>Free<span className='text-blue-400 bold'>Chronicle</span></h1>
        ...
        <button onClick={recordingStatus === 'recording' ? 
          ...
          <div className='flex items-center gap-2'>
            {duration !== 0 && (
              <p className='text-sm text-slate-500'>{duration}s</p>)
            }
            <i className={"fa-solid duration-200 fa-microphone text-blue-400 " + (recordingStatus === 'recording' ? ' text-rose-400' : 
            "")}></i>
          </div>
        </button>
        <p className='text-base'>Or <label className='text-blue-400 cursor-pointer hover:text-blue-600 duration-200'>uplodad 
          <input onChange={(e) => {
