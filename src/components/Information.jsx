...
export default function Information(props) {
  ...
  return (
    <main className='flex-1 p-4 flex flex-col gap-3 text-center sm:gap-4 justify-center pb-20 max-w-prose w-full mx-auto'>
        ...
        md:text-6xl whitespace-nowrap'>Your <span className='text-blue-900 bold'>Transcription</span></h1>
        <div className='grid grid-cols-2 mx-auto bg-white shadow rounded-full overflow-hidden items-center'>
            <button onClick={() => {
                setTab('transcription')
            }} className={'px-4 duration-200 py-1 font-medium ' + 
                (tab === 'transcription' ? 
                ' bg-blue-300 text-white' : ' text-blue-400 hover:text-blue-900')}>Transcription</button>
            <button onClick={() => {
                setTab('translation')
            }} className={'px-4 duration-200 py-1 font-medium ' + 
                (tab === 'translation' ? 
                ' bg-blue-300 text-white' : ' text-blue-400 hover:text-blue-900')}>Translation</button>
        </div>
        ...
        <div className='flex items-center gap-4 mx-auto text-base'>
            <button onClick={handleCopy} title='Copy' className='bg-yellow-50 hover:text-blue-900 duration-200 text-blue-400 px-2 aspect-square grid place-items-center rounded-md px-4'>
                <i className="fa-regular fa-copy"></i>
            </button>
            <button onClick={handleDownload} title='Download' className='bg-yellow-50 hover:text-blue-900 duration-200 text-blue-400 px-2 aspect-square grid place-items-center rounded-md px-4'>
